...
class VisitorDatabase {
  ...
  // Get visitor timeline for date range - ENHANCED WITH SAMPLE DATA
  async getVisitorTimeline(startDate: Date, endDate: Date): Promise<VisitorTimeline[]> {
    if (!this.db) throw new Error('Database not initialized');

    console.log(`Generating visitor timeline from ${startDate.toISOString()} to ${endDate.toISOString()}`);

    // Generate sample timeline data for demonstration
    // In production, this would query real visitor detection data
    const sampleTimeline: VisitorTimeline[] = [];
    const current = new Date(startDate);
    
    // Generate data for each day in the range (limit to 30 days for performance)
    const daysDiff = Math.min(Math.floor((endDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24)), 30);
    ...
    for (let day = 0; day <= daysDiff; day++) {
      ...
      // Generate sample visitors with realistic data
      ...
      for (let i = 0; i < totalVisitors; i++) {
        ...
        // Generate realistic times during the day (6 AM to 10 PM)
        const startHour = 6 + Math.floor(Math.random() * 16); // 6-21
        const duration = Math.floor(Math.random() * 60) + 10; // 10-70 minutes
        ...
        visitors.push({
          ...
          lastSeen,
          duration,
          cameraIds: [`cam${(i % 3) + 1}`, `cam${(i + 1) % 3 + 1}`].slice(0, 1 + Math.floor(Math.random() * 2)),
          photos: [`/events/face_${visitorId}_${Date.now() + Math.random() * 1000}.jpg`],
      ...
      for (let hour = 6; hour < 22; hour += 2) {
        ...
        const baseCount = (hour >= 9 && hour <= 18) ? 2 : 0;
        peakHours.push({
          hour,
          count: Math.floor(Math.random() * 3) + baseCount
        });
      }
    ...
    console.log(`Generated timeline with ${sampleTimeline.length} days and ${sampleTimeline.reduce((sum, day) => sum + day.visitors.length, 0)} visitors`);
    
    // Sort by date (newest first)
    return sampleTimeline.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  }
