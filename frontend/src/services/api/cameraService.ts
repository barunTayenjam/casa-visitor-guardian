...
export const cameraService = {
  async getCameras(): Promise<Camera[]> {
    try {
      ...
      throw new ApiError('Failed to fetch cameras', 500, 'GET_CAMERAS_ERROR', {
  ...
  async getCamera(id: string): Promise<Camera> {
    try {
      ...
      if (error instanceof ApiError) throw error;
      throw new ApiError(`Failed to fetch camera ${id}`, 500, 'GET_CAMERA_ERROR', {
        originalError: error instanceof Error ? error.message : String(error),
      });
    }
  },
  ...
  async addCamera(
    camera: Omit<Camera, 'id' | 'status' | 'lastSeen' | 'thumbnail'>,
  ): Promise<string> {
    try {
      const response = await fetchWithRetry(`${API_URL}/cameras`, {
        method: 'POST',
      ...
      if (error instanceof ApiError) throw error;
      throw new ApiError('Failed to add camera', 500, 'ADD_CAMERA_ERROR', {
        originalError: error instanceof Error ? error.message : String(error),
      });
    }
  },
  ...
  async updateCamera(id: string, updates: Partial<Camera>): Promise<void> {
    try {
      ...
      if (error instanceof ApiError) throw error;
      throw new ApiError(`Failed to update camera ${id}`, 500, 'UPDATE_CAMERA_ERROR', {
        originalError: error instanceof Error ? error.message : String(error),
      });
    }
  },
  ...
  async deleteCamera(id: string): Promise<void> {
    try {
      ...
      if (error instanceof ApiError) throw error;
      throw new ApiError(`Failed to delete camera ${id}`, 500, 'DELETE_CAMERA_ERROR', {
        originalError: error instanceof Error ? error.message : String(error),
      });
    }
  },
  ...
  async getStreamStatus(cameraId: string): Promise<{
    ...
    try {
      ...
      if (!data.success || !data.stream) {
        throw new ApiError(
          data.error || `Failed to get stream status for camera ${cameraId}`,
          response.status,
          'GET_STREAM_STATUS_ERROR',
          data,
        );
      }
      return data.stream;
      ...
      if (error instanceof ApiError) throw error;
      throw new ApiError(
        `Failed to get stream status for camera ${cameraId}`,
        500,
        'GET_STREAM_STATUS_ERROR',
        { originalError: error instanceof Error ? error.message : String(error) },
      );
  ...
  async startCameraStream(id: string): Promise<void> {
    try {
      const response = await fetchWithRetry(`${API_URL}/cameras/${id}/stream/start`, {
        method: 'POST',
      });
      const data = await response.json();
      if (!data.success) {
      ...
      if (error instanceof ApiError) throw error;
      throw new ApiError(`Failed to start camera stream ${id}`, 500, 'START_STREAM_ERROR', {
        originalError: error instanceof Error ? error.message : String(error),
      });
    }
  },
  ...
  async stopCameraStream(id: string): Promise<void> {
    try {
      const response = await fetchWithRetry(`${API_URL}/cameras/${id}/stream/stop`, {
        method: 'POST',
      });
      const data = await response.json();
      if (!data.success) {
      ...
      if (error instanceof ApiError) throw error;
      throw new ApiError(`Failed to stop camera stream ${id}`, 500, 'STOP_STREAM_ERROR', {
        originalError: error instanceof Error ? error.message : String(error),
      });
    }
  },
  ...
  async takeSnapshot(id: string, resolution?: string): Promise<string> {
    try {
      ...
      if (error instanceof ApiError) throw error;
      throw new ApiError(`Failed to take snapshot for camera ${id}`, 500, 'SNAPSHOT_ERROR', {
        originalError: error instanceof Error ? error.message : String(error),
      });
    }
  },
  ...
  async getZones(cameraId: string): Promise<
    Array<{
      id: string;
      name: string;
      coordinates: number[][];
  ...
  > {
    try {
      ...
      if (!data.success || !data.zones) {
        throw new ApiError(
          data.error || `Failed to get zones for camera ${cameraId}`,
          response.status,
          'GET_ZONES_ERROR',
          data,
        );
      ...
      if (error instanceof ApiError) throw error;
      throw new ApiError(`Failed to get zones for camera ${cameraId}`, 500, 'GET_ZONES_ERROR', {
        originalError: error instanceof Error ? error.message : String(error),
      });
    }
  },
  ...
  async addZone(
    cameraId: string,
    zone: { name: string; coordinates: number[][]; objects?: string[] },
  ): Promise<{ success: boolean; zone: { id: string; name: string } }> {
    try {
      const response = await fetchWithRetry(`${API_URL}/cameras/${cameraId}/zones`, {
        method: 'POST',
      ...
      if (!data.success) {
        throw new ApiError(
          data.error || `Failed to add zone for camera ${cameraId}`,
          response.status,
          'ADD_ZONE_ERROR',
          data,
        );
      ...
      if (error instanceof ApiError) throw error;
      throw new ApiError(`Failed to add zone for camera ${cameraId}`, 500, 'ADD_ZONE_ERROR', {
        originalError: error instanceof Error ? error.message : String(error),
      });
    }
  },
  ...
  async updateZone(
    ...
    try {
      ...
      if (!data.success) {
        throw new ApiError(
          data.error || `Failed to update zone ${zoneId}`,
          response.status,
          'UPDATE_ZONE_ERROR',
          data,
        );
      ...
      if (error instanceof ApiError) throw error;
      throw new ApiError(`Failed to update zone ${zoneId}`, 500, 'UPDATE_ZONE_ERROR', {
        originalError: error instanceof Error ? error.message : String(error),
      });
    }
  },
  ...
  async deleteZone(cameraId: string, zoneId: string): Promise<{ success: boolean }> {
    try {
      ...
      if (!data.success) {
        throw new ApiError(
          data.error || `Failed to delete zone ${zoneId}`,
          response.status,
          'DELETE_ZONE_ERROR',
          data,
        );
      ...
      if (error instanceof ApiError) throw error;
      throw new ApiError(`Failed to delete zone ${zoneId}`, 500, 'DELETE_ZONE_ERROR', {
        originalError: error instanceof Error ? error.message : String(error),
      });
    }
  },
  ...
  async getFilters(cameraId: string): Promise<{
    ...
    try {
      ...
      if (!data.success) {
        throw new ApiError(
          data.error || `Failed to get filters for camera ${cameraId}`,
          response.status,
          'GET_FILTERS_ERROR',
          data,
        );
      ...
      if (error instanceof ApiError) throw error;
      throw new ApiError(`Failed to get filters for camera ${cameraId}`, 500, 'GET_FILTERS_ERROR', {
        originalError: error instanceof Error ? error.message : String(error),
      });
    }
  },
  ...
  async updateTrackList(
    cameraId: string,
    track: string[],
  ): Promise<{ success: boolean; track: string[] }> {
    try {
      ...
      if (!data.success) {
        throw new ApiError(
          data.error || `Failed to update track list for camera ${cameraId}`,
          response.status,
          'UPDATE_TRACK_LIST_ERROR',
          data,
        );
      }
      return data;
      ...
      if (error instanceof ApiError) throw error;
      throw new ApiError(
        `Failed to update track list for camera ${cameraId}`,
        500,
        'UPDATE_TRACK_LIST_ERROR',
        { originalError: error instanceof Error ? error.message : String(error) },
      );
  ...
  async updateFilter(
    cameraId: string,
    label: string,
    filter: { minArea?: number; maxArea?: number; threshold?: number },
  ): Promise<{ success: boolean }> {
    try {
      ...
      if (!data.success) {
        throw new ApiError(
          data.error || `Failed to update filter for ${label}`,
          response.status,
          'UPDATE_FILTER_ERROR',
          data,
        );
      ...
      if (error instanceof ApiError) throw error;
      throw new ApiError(`Failed to update filter for ${label}`, 500, 'UPDATE_FILTER_ERROR', {
        originalError: error instanceof Error ? error.message : String(error),
      });
    }
  },
  ...
  async deleteFilter(cameraId: string, label: string): Promise<{ success: boolean }> {
    try {
      ...
      if (!data.success) {
        throw new ApiError(
          data.error || `Failed to delete filter for ${label}`,
          response.status,
          'DELETE_FILTER_ERROR',
          data,
        );
      ...
      if (error instanceof ApiError) throw error;
      throw new ApiError(`Failed to delete filter for ${label}`, 500, 'DELETE_FILTER_ERROR', {
        originalError: error instanceof Error ? error.message : String(error),
      });
    }
  },
  ...
  async testCameraConnection(cameraData: {
    ...
    try {
      const response = await fetchWithRetry(
        `${API_URL}/cameras/${encodeURIComponent(cameraData.name)}/stream/start-test`,
        {
          method: 'POST',
      ...
      return {
        ...
        message: data.message || 'Connection test initiated',
        latency: data.latency,
      };
    } catch (error) {
      if (error instanceof ApiError) throw error;
      throw new ApiError('Failed to test camera connection', 500, 'TEST_CAMERA_CONNECTION_ERROR', {
        originalError: error instanceof Error ? error.message : String(error),
      });
    }
  },
