...
class Logger {
  ...
  constructor() {
    ...
    this.enableConsoleLogging =
      import.meta.env.DEV || localStorage.getItem('enableConsoleLogging') !== 'false';
  ...
  apiResponse(
    ...
    this.info(`${method} ${url} - ${status}`, 'API', {
      method,
      url,
      status,
      responseTime,
  ...
  performance(metric: string, value: number, unit?: string, metadata?: LogMetadata) {
    this.debug(`Performance: ${metric} = ${value}${unit ? ' ' + unit : ''}`, 'PERFORMANCE', {
      metric,
      value,
      unit,
      ...metadata,
  ...
  cameraAction(action: string, cameraId: string, metadata?: LogMetadata) {
    this.info(`Camera action: ${action} for camera ${cameraId}`, 'CAMERA', {
      action,
      cameraId,
      ...metadata,
    });
