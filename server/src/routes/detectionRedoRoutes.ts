  ...
  validateBody(
    z.object({
      filename: z
        ...
        .max(255)
        .optional()
        .refine((v) => !(v && v.includes('..')), 'Path traversal not allowed'),
      filepath: z
        .string()
        .max(500)
        .optional()
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const vehicleCount = validDetections.filter((d) =>
      ...
      WHERE id = $5`,
        [
          ...
          JSON.stringify(validDetections),
          Math.round(
            validDetections.reduce((acc, d) => acc + (d.confidence || 0), 0) /
