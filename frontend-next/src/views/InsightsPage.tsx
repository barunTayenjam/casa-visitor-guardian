            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={objectData} dataKey="value" nameKey="name" innerRadius={50} outerRadius={85}
                  paddingAngle={2} strokeWidth={0} label={({ name, percent }) => `${name} ${((percent ?? 0) * 100).toFixed(0)}%`}>
                  {objectData.map((_, i) => (
                    <Cell key={i} fill={[SERIES.person, SERIES.vehicle, SERIES.motion, '#9085e9', '#e66767'][i % 5]} />
                  ))}
                </Pie>
                <Tooltip contentStyle={tooltipStyle} formatter={(value, name) => [`${Number(value ?? 0).toLocaleString()} detections`, String(name ?? '')]} />
              </PieChart>
            </ResponsiveContainer>
          </div>
        </Card>
          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip contentStyle={tooltipStyle} formatter={(value, name) => [`${Number(value ?? 0).toLocaleString()} events`, String(name ?? '')]} />
              </PieChart>
            </ResponsiveContainer>
          </div>
        </Card>
