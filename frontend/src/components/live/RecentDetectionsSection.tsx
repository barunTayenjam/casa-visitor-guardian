...
function getPrimaryObject(event: {
  labels?: string[];
  personCount?: number;
  objectDetections?: Array<{ class: string }>;
}): { type: string; icon: React.ElementType } {
  if (event.personCount && event.personCount > 0) {
    return { type: 'Person', icon: User };
  }
  if (event.objectDetections && event.objectDetections.length > 0) {
    return { type: event.objectDetections[0].class, icon: Eye };
  }
...
export const RecentDetectionsSection: React.FC<RecentDetectionsSectionProps> = ({
    ...
    const fetchRecent = async () => {
      try {
        ...
        const items: DetectionItem[] = (data.events || []).map((ev: { id: string; imageUrl?: string; image_path?: string; timestamp: string; event_type?: string; confidence?: number; labels?: string[]; personCount?: number; objectDetections?: Array<{ class: string }> }) => {
          ...
          return {
            id: ev.id,
            imageUrl: ev.imageUrl || ev.image_path || null,
    ...
    return (
      <div className={cn('flex items-center justify-center py-3', className)}>
        <div className="flex gap-2">
          {[0, 1, 2].map((i) => (
            <div key={i} className="h-12 w-12 rounded bg-muted animate-pulse" />
          ))}
        </div>
      </div>
    );
  ...
  if (detections.length === 0) {
    return (
      <div
        className={cn(
          'flex flex-col items-center justify-center py-4 text-muted-foreground',
          className
        )}
        role="status"
      ...
      {detections.map((det) => {
        ...
        return (
          <button
            key={det.id}
            role="listitem"
            className="flex-1 min-w-0 rounded-md border border-border bg-muted/40 overflow-hidden hover:bg-muted/70 transition-colors text-left focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-1"
          ...
          >
            <div className="aspect-video bg-muted flex items-center justify-center overflow-hidden">
              {det.imageUrl ? (
                <img
                  src={det.imageUrl}
                  alt={`${det.objectType} detection`}
                  className="h-full w-full object-cover"
                  loading="lazy"
                />
              ) : (
                <ObjectIcon className="h-5 w-5 text-muted-foreground" />
              )}
            </div>
            <div className="px-1.5 py-1">
              <div className="flex items-center gap-1">
                <ObjectIcon className="h-3 w-3 text-muted-foreground shrink-0" />
                <span className="text-[10px] font-medium truncate">{det.objectType}</span>
              </div>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-0.5 text-muted-foreground">
                  <Clock className="h-2.5 w-2.5" />
                  <span className="text-[9px]">{formatRelativeTime(det.timestamp)}</span>
                </div>
                {det.confidence > 0 && (
